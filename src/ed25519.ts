...
export const ed25519: Ed25519 = {
  ...
  verify(publicKey, signature, message) {
    ...
    try {
      ...
      // small-order checks `verify_strict` makes, then the equation. The
      // reference's decoder reduces a non-canonical y instead; the outcome is
      // the same `false`, because an undecodable key is `false` on both sides
      // and a decodable non-canonical key is small-order (rejected below and
      // by `verify_strict`) or a point whose discrete logarithm nobody knows.
