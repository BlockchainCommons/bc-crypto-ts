...
export function chacha20(
  ...
  nonce: Uint8Array,
  data: Uint8Array,
  options?: Chacha20Options,
): Uint8Array<ArrayBuffer> {
  requireLength("ChaCha20 key", key, 32);
  requireLength("ChaCha20 nonce", nonce, 12);
  requireBytes("ChaCha20 data", data);
  requireOptions("ChaCha20 options", options, true);
  const counter = expectInt("ChaCha20 counter", options?.counter ?? 0, 0, U32_MAX - 1);
  ...
  // noble writes into a fresh `Uint8Array(data.length)`, an ArrayBuffer-backed view.
  return guard(
    () => nobleChacha20(key, nonce, data, undefined, counter) as Uint8Array<ArrayBuffer>,
