/**
 * Build-agnostic recipes for the crypto vector suites (Phase 1.1).
 *
 * Every argument is named, so the option-object refactor in Phase 3 changes
 * only the adapters. Byte inputs are hex or cycling-byte descriptions; the
...
/** The working tree. Rewritten in Phase 3 for the new names. */
export function redesignedAdapterFor(m: any, rand: any): VectorApi {
  if (typeof m.aeadChaCha20Poly1305Encrypt === "function") {
    // Pre-Phase-3 surface, but the redesigned rand: build a generator whose
    // fillBytes drives the old `*Using` functions via randomBytes(n, { rng }).
  ...
  return {
    ...
    deriveAgreement: m.x25519.deriveAgreementPrivateKey,
    deriveSigning: m.deriveSigningPrivateKey,
    ecdsaDerive: m.ecdsa.derivePrivateKey,
