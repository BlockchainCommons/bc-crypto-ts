...
describe("Hash functions", () => {
  ...
  test("test_hkdf_hmac_sha256", () => {
    ...
    const derivedKey = hkdfSha256(keyMaterial, salt, { dkLen: keyLen });
...
describe("ChaCha20 keystream", () => {
  // RFC 8439 §2.4.2: the "Ladies and Gentlemen" plaintext with counter 1.
  const key = Uint8Array.from({ length: 32 }, (_, i) => i);
  const nonce = Uint8Array.from([0, 0, 0, 0, 0, 0, 0, 0x4a, 0, 0, 0, 0]);
  const plaintext = new TextEncoder().encode(
    "Ladies and Gentlemen of the class of '99: If I could offer you only one tip for the future, sunscreen would be it.",
  );
  it("matches the RFC 8439 vector at counter 1", () => {
    const out = chacha20(key, nonce, plaintext, { counter: 1 });
    expect(Buffer.from(out.subarray(0, 16)).toString("hex")).toBe(
      "6e2e359a2568f98041ba0728dd0d6981",
    );
    expect(Buffer.from(out.subarray(out.length - 16)).toString("hex")).toBe(
      "0bbf74a35be6b40b8eedf2785e42874d",
    );
  });
  it("is its own inverse and checks the key and nonce lengths", () => {
    const out = chacha20(key, nonce, plaintext);
    expect(chacha20(key, nonce, out)).toEqual(plaintext);
    expect(() => chacha20(new Uint8Array(31), nonce, plaintext)).toThrow();
    expect(() => chacha20(key, new Uint8Array(8), plaintext)).toThrow();
  });
});
